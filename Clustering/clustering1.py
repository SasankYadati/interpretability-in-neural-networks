...
def output_class_prediction(encodings,number_of_classes):
	plt.figure()
	print(encodings)
	filename = "output_class_pred.png"
...
def draw_plot(c_r):
  plt.figure()
  print(c_r)
  filename = "concepts_relevance_scores.png"
  null = []
  for i in c_r:
    if(c_r[i] == [0.0,0.0]):
      null.append(i)
...
def draw_plot_encodings(c1,c2):
  ...
  filename="encodings_noisy_and_original.png"
  plt.savefig(filename)
  #plt.show()
...
def get_similar_images_image(l,concept_images,x_train):
	...
	for i in range(l):
	  for j in range(l):
	    ...
	    m+=1
	  k+=1
	filename = "grounding_elements.png"
	plt.savefig(filename)
...
if __name__ == '__main__':
	...
	while(True):
		...
		x_ = x_test[ip_sample:ip_sample+1]
		...
		concepts_image_filename = "concepts1.png"
